...
3)//타임아웃 설정 -  timeout : milliseconds
socket.connect(socketAddress, 5000); 
※connect 메서드에는 sockeAddress가 와야하는데
 그 객체가 추상클래스라는것은 그 추상클래스를 상속받은 서브클래스를 호출하라는것이다.
```
...
Socket socket = ss.accept(); //대기열에서 꺼낸다
System.out.println("대기 중인 클라이언트 중 한 개의 클라이언트에 대해 연결 승인!");
//대기열에 있는상태에서 클라이언트의 연결을 끊어도 대기열엔 클라이언트의 기록이 되어있어서 비워지지않는다
//accept()될때까지 대기열은 안비워진다
...
```java
       // => read()메서드는 클라이언트가 1바이트를 읽을 때까지 리턴하지 않는다.
...
```
printStream은 byte Stream   - flush()자동호출
 character Stream 에서 buf를 사용하지않을때는 auto flush()
 character Stream 에서 buf를 사용시 flush()호출해야 한다.
