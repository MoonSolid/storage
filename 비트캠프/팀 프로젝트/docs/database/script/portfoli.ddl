...
-- 일반회원 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_general_member
	ON pf_general_member ( -- 일반회원
	);

-- 일반회원 인덱스
CREATE INDEX IX_pf_general_member
	ON pf_general_member( -- 일반회원
	);
...
-- 기업회원 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_company_member
	ON pf_company_member ( -- 기업회원
	);

-- 기업회원 인덱스
CREATE INDEX IX_pf_company_member
	ON pf_company_member( -- 기업회원
	);
...
-- 포트폴리오 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_portfolio
	ON pf_portfolio ( -- 포트폴리오
	);

-- 포트폴리오 인덱스
CREATE INDEX IX_pf_portfolio
	ON pf_portfolio( -- 포트폴리오
	);
...
CREATE TABLE pf_job_posting (
	...
	readable             INTEGER      NOT NULL COMMENT '공개여부', -- 공개여부
	minimum_education_no INTEGER      NULL     COMMENT '최소학력번호', -- 최소학력번호
...
-- 채용공고 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_job_posting
	ON pf_job_posting ( -- 채용공고
	);

-- 채용공고 인덱스
CREATE INDEX IX_pf_job_posting
	ON pf_job_posting( -- 채용공고
	);
...
CREATE TABLE pf_report (
	...
	status          VARCHAR NOT NULL COMMENT '처리상태' -- 처리상태
)
COMMENT '신고';
...
-- 신고 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_report
	ON pf_report ( -- 신고
	);

-- 신고 인덱스
CREATE INDEX IX_pf_report
	ON pf_report( -- 신고
	);
...
-- 회원쪽지 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_message
	ON pf_message ( -- 회원쪽지
	);

-- 회원쪽지 인덱스
CREATE INDEX IX_pf_message
	ON pf_message( -- 회원쪽지
	);
...
-- 배너 유니크 인덱스
CREATE UNIQUE INDEX UIX_pf_banner
	ON pf_banner ( -- 배너
	);

-- 배너 인덱스
CREATE INDEX IX_pf_banner
	ON pf_banner( -- 배너
	);
...
CREATE TABLE pf_payment (
	...
	status     VARCHAR  NOT NULL COMMENT '결제상태', -- 결제상태
	date       DATETIME NOT NULL DEFAULT now() COMMENT '결제일', -- 결제일
	method     VARCHAR  NOT NULL COMMENT '결제수단' -- 결제수단
)
COMMENT '결제';
...
CREATE TABLE pf_county_district (
	...
	county_name        <데이터 타입 없음> NULL     COMMENT '군이름', -- 군이름
	district_name      <데이터 타입 없음> NULL     COMMENT '구이름' -- 구이름
)
COMMENT '군구';
...
CREATE UNIQUE INDEX UIX_pf_employment_status
	ON pf_employment_status ( -- 고용형태
		employment_status_name ASC -- 고용형태이름
	);
...
-- 일반회원학력전공
ALTER TABLE pf_general_member_education_major
	ADD CONSTRAINT FK_pf_general_member_education_TO_pf_general_member_education_major -- 일반회원학력 -> 일반회원학력전공
		FOREIGN KEY (
			general_member_no, -- 일반회원번호
			education_no       -- 학력번호
		)
