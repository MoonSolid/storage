ValueObject = Domain = Entity

스프링에서는 int값을 null값으로 바꿔주지않는다. 그래서 기본값을 설정해준다.

```
@RequestParam(defaultValue = "0") defaultVale 설정
```
...
파라미터벨류에 required=false 로 설정하거나 ""등으로 빈 문자열이라도 받는다.
...
loadonstartup 태그밑에  multipart-config 태그를 넣어줘야한다.
